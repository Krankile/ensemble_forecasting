...
class M4EnsembleData(Dataset):
    def __init__(
        self, meta_path, feature_set, n_models, subset=None, normalize="standard"
    ):
        ...
        (
            ...
            self.forecast,
            self.actuals,
        ) = feature_extractor(meta_df, feature_set, n_models, normalize)
...
def ensemble_loaders(
    ...
    data1 = M4EnsembleData(
        datapath, feature_set, n_models, subset=train_idxs, normalize=normalize
    )
    loader1 = DataLoader(
        data1,
...
def ensemble_loaders_kfold(
    ...
    data2 = M4EnsembleData(val, feature_set, n_models, normalize=normalize)
