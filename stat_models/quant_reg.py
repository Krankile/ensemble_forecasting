...
def quant_reg_prediction(time_series, meta_variables, q=0.95, visualise=False):
    ...
    future_t = sm.add_constant(
        list(range(len(time_series), len(time_series) + meta_variables["h"]))
    )
