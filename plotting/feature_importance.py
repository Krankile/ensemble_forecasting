...
def plot_feature_importance(importances, *, savepath=None, useerror=None, show=True, normalize=False, figheight=12, yticksize=10):
    ...
    plt.barh(*zip(*filter(lambda x: "lstm" in x[0], zip(lab, vals))), alpha=1, color=main, xerr=lstm_err, label="LSTM AE features")
    plt.barh(*zip(*filter(lambda x: "lstm" not in x[0], zip(lab, vals))), alpha=1, color=main2, xerr=stat_err, label="Statistical features")
